...
const Gallery = ({ images }) => (_jsxs("div", { style: {
    ...
    ` } }), images.map((img, idx) => (_jsx(motion.img, { src: img, alt: `Memory ${idx + 1}`, initial: { opacity: 0, y: 50 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true, amount: 0.3 }, transition: { delay: 0.1 + idx * 0.08, duration: 0.7, ease: "easeOut" }, whileHover: {
                scale: 1.03,
